...
- Use Google Drive for study memory only when the user explicitly says Google Drive. Verify that the Google Drive plugin is available; if it is unavailable or disconnected, notify the user and do not silently fall back to local storage. Save a matching canonical JSON and human-readable Markdown pair into the existing folder the user identifies or the confirmed folder they request. Prefer JSON for later structured lookup, use Markdown as the readable companion, and count the pair as one record. A Google Drive save replaces the local save for that record, so do not keep persistent local copies or a local index entry.
